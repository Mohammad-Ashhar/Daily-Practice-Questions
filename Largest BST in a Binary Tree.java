	...
	static class BinaryTree {
		...
		private Info largestBSTinBT(Node root) {
			...
			if (leftInfo.isBST && rightInfo.isBST && root.data > leftInfo.max && root.data < rightInfo.min) {
				curr.isBST = true;
				curr.min = Math.min(leftInfo.min, root.data == Integer.MIN_VALUE ? leftInfo.min : root.data);
				curr.min = (leftInfo.min == Integer.MAX_VALUE) ? root.data : Math.min(leftInfo.min, root.data);
				curr.max = (rightInfo.max == Integer.MIN_VALUE) ? root.data : Math.max(rightInfo.max, root.data);
